...
module Gramophone.GUI.ListArtists
       (
         getListArtistsR
       ) where
...
getListArtistsR :: Handler Html
getListArtistsR = defaultLayout $ do
  artists <- withDatabase $ do
    ids <- getAllArtists
    catMaybes <$> mapM getArtist ids
  dbTableWidget [artistNameColumn] artists
